...
%include "namespace grammar { namespace where {"
%code "}}"
...
identifier(V) ::= identifier(I) DOT FIELD(F) .
{
...
expr(R) ::= expr(A) IS expr(B) .
{
    R = create_is_operator(A, B);
    add_to_temp_set(R, state->temp_set);
    remove_from_temp_set(A, state->temp_set);
    remove_from_temp_set(B, state->temp_set);
}
expr(R) ::= expr(A) IS_NOT expr(B) .
{
    R = create_is_operator(A, B);
    R = create_not_operator(R);
    add_to_temp_set(R, state->temp_set);
    remove_from_temp_set(R, state->temp_set);
    remove_from_temp_set(B, state->temp_set);
}
...
expr(R) ::= expr(A) LIKE expr(B) . 
{
    R = create_like_operator(A, B);
    add_to_temp_set(R, state->temp_set);
    remove_from_temp_set(A, state->temp_set);
    remove_from_temp_set(B, state->temp_set);
}
